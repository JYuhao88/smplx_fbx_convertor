...
def CreatePatch(pSdkManager, pName):
    ...
    lPatch.InitControlPoints(4, FbxPatch.EType.eBSpline, 7, FbxPatch.EType.eBSpline)
    lPatch.SetStep(4, 4)
...
def LinkPatchToSkeleton(pSdkManager, pPatchNode, pSkeletonRoot):
    ...
    lClusterToRoot.SetLinkMode(FbxCluster.ELinkMode.eTotalOne)
    for i in range(0, 16):
    ...
    lClusterToLimbNode1.SetLinkMode(FbxCluster.ELinkMode.eTotalOne)
    for i in range(4, 24):
    ...
    lClusterToLimbNode2.SetLinkMode(FbxCluster.ELinkMode.eTotalOne)
...
def StoreBindPose(pSdkManager, pScene, pPatchNode, pSkeletonRoot):
    ...
    if pPatchNode and pPatchNode.GetNodeAttribute():
        ...
        if lNodeAttributeType in (FbxNodeAttribute.EType.eMesh, FbxNodeAttribute.EType.eNurbs, FbxNodeAttribute.EType.ePatch):
            lSkinCount = pPatchNode.GetNodeAttribute().GetDeformerCount(FbxDeformer.EDeformerType.eSkin)
            for i in range(lSkinCount):
                lSkin = pPatchNode.GetNodeAttribute().GetDeformer(i, FbxDeformer.EDeformerType.eSkin)

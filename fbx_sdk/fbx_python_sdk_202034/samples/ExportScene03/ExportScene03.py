...
def CreateCubeWithTexture(pSdkManager, pName):
    ...
    lTextureDiffuseLayer.SetMappingMode(FbxLayerElement.EMappingMode.eByPolygon)
    lTextureDiffuseLayer.SetReferenceMode(FbxLayerElement.EReferenceMode.eIndexToDirect)
    lLayer.SetTextures(FbxLayerElement.EType.eTextureDiffuse, lTextureDiffuseLayer)

    # Set texture mapping for ambient channel.
    lTextureAmbientLayer=FbxLayerElementTexture.Create(lMesh, "Ambient Textures")
    lTextureAmbientLayer.SetMappingMode(FbxLayerElement.EMappingMode.eByPolygon)
    lTextureAmbientLayer.SetReferenceMode(FbxLayerElement.EReferenceMode.eIndexToDirect)
    lLayer.SetTextures(FbxLayerElement.EType.eTextureAmbient, lTextureAmbientLayer)

    # Set texture mapping for emissive channel.
    lTextureEmissiveLayer=FbxLayerElementTexture.Create(lMesh, "Emissive Textures")
    lTextureEmissiveLayer.SetMappingMode(FbxLayerElement.EMappingMode.eByPolygon)
    lTextureEmissiveLayer.SetReferenceMode(FbxLayerElement.EReferenceMode.eIndexToDirect)
    lLayer.SetTextures(FbxLayerElement.EType.eTextureEmissive, lTextureEmissiveLayer)
    ...
    lUVAmbientLayer.SetMappingMode(FbxLayerElement.EMappingMode.eByPolygonVertex)
    lUVAmbientLayer.SetReferenceMode(FbxLayerElement.EReferenceMode.eIndexToDirect)
    lLayer.SetUVs(lUVAmbientLayer, FbxLayerElement.EType.eTextureAmbient)
    ...
    lUVEmissiveLayer.SetMappingMode(FbxLayerElement.EMappingMode.eByPolygonVertex)
    lUVEmissiveLayer.SetReferenceMode(FbxLayerElement.EReferenceMode.eIndexToDirect)
    lLayer.SetUVs(lUVEmissiveLayer, FbxLayerElement.EType.eTextureEmissive)
...
def CreatePyramidWithMaterials(pSdkManager, pName):
    ...
    lNormalLayer.SetMappingMode(FbxLayerElement.EMappingMode.eByControlPoint)
    lNormalLayer.SetReferenceMode(FbxLayerElement.EReferenceMode.eDirect)
    ...
    lMaterialLayer.SetMappingMode(FbxLayerElement.EMappingMode.eByPolygon)
    lMaterialLayer.SetReferenceMode(FbxLayerElement.EReferenceMode.eIndexToDirect)
...
def MapVertexCacheOnTriangle(pSdkManager, pTriangle, pSampleFileName):
    ...
    if gExportVertexCacheMCFormat:
        lCache.SetCacheFileFormat(FbxCache.EFileFormat.eMayaCache)
    else:
        lCache.SetCacheFileFormat(FbxCache.EFileFormat.eMaxPointCacheV2)
...
def AnimateVertexCacheOnTriangleDoubleVertex(pTriangle, pFrameRate):
    ...
    lDeformer = pTriangle.GetGeometry().GetDeformer(0, FbxDeformer.EDeformerType.eVertexCache)
    ...
    if gExportVertexCacheMCFormat:
        lRet = lCache.OpenFileForWrite(FbxCache.EMCFileCount.eMCOneFile, pFrameRate, pTriangle.GetName(), FbxCache.EMCBinaryFormat.eMCX)
    else:
...
def AnimateVertexCacheOnTriangleInt32(pTriangle, pFrameRate):
    ...
    if gExportVertexCacheMCFormat:
        lRet = lCache.OpenFileForWrite(FbxCache.EMCFileCount.eMCOneFile, pFrameRate, pTriangle.GetName(), FbxCache.EMCBinaryFormat.eMCX, FbxCache.kInt32Array)

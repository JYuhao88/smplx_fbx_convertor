...
def CreateLight(pSdkManager, pName):
    ...
    lLight.LightType.Set(FbxLight.EType.eSpot.value)
...
def CreateCamera(pSdkManager, pName):
    ...
    lCamera.SetApertureMode(FbxCamera.EApertureMode.eVertical)
...
def AnimateLightGroup(pLightGroup, pAnimLayer):
    ...
    if lCurve:
        ...
        lCurve.KeySetInterpolation(lKeyIndex, FbxAnimCurveDef.EInterpolationType.eInterpolationLinear)
...
def AnimateLight(pLight, pIndex, pAnimLayer):
    ...
    # X rotation swoops & cone angle woobles.
        ...
        for i in range(8):
            ...
            lConeCurve.KeySetInterpolation(lKeyIndex, FbxAnimCurveDef.EInterpolationType.eInterpolationLinear)
    ...
    # Color cycling.
        ...
        if lCurve[0] and lCurve[1] and lCurve[2]:
            ...
            for i in range(24):
                ...
                lCurve[0].KeySetInterpolation(lKeyIndex, FbxAnimCurveDef.EInterpolationType.eInterpolationCubic)

                lKeyIndex = lCurve[1].KeyAdd(lTime)[0]
                lCurve[1].KeySetValue(lKeyIndex, lColor[j][1])
                lCurve[1].KeySetInterpolation(lKeyIndex, FbxAnimCurveDef.EInterpolationType.eInterpolationCubic)

                lKeyIndex = lCurve[2].KeyAdd(lTime)[0]
                lCurve[2].KeySetValue(lKeyIndex, lColor[j][2])
                lCurve[2].KeySetInterpolation(lKeyIndex, FbxAnimCurveDef.EInterpolationType.eInterpolationCubic)
...
def AnimateCamera(pCamera, pAnimLayer):
    ...
    if lCurve:
        ...
        lTime.SetSecondDouble(0.0)
        lKeyIndex = lCurve.KeyAdd(lTime)[0]
        lCurve.KeySet(lKeyIndex, lTime, 0.0, FbxAnimCurveDef.EInterpolationType.eInterpolationLinear)
...
def AnimateCameraSwitcher(pCameraSwitcher, pAnimLayer):
    ...
    if lCurve:
        ...
        lCurve.KeySetInterpolation(lKeyIndex, FbxAnimCurveDef.EInterpolationType.eInterpolationConstant)
